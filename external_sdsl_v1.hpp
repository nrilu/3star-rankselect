...
class external_sdsl_v1 : public Bitvector {

	sdsl::bit_vector bv;
	sdsl::bit_vector::rank_1_type rank_v1;
...
public:
	...
	void build_auxiliaries() override {
		...
		rank_v1 = sdsl::bit_vector::rank_1_type(&bv);
	}

	uint64_t select_1(int64_t k) const override {
		cout << "select_1: not supported by sdsl_rank_v1" << endl;
		return 0;
	}

	uint64_t select_0(int64_t k) const override {
		cout << "select_0: not supported by sdsl_rank_v1" << endl;
		return 0;
	}

	uint64_t rank_1(const uint64_t i) const override {
		return rank_v1.rank(i);
	}
	...
	uint64_t space_in_bits() const override {
		...
		int64_t r_bits  = 8*sdsl::size_in_bytes(rank_v1);
		int64_t total   = bv_bits + r_bits;
		if(show_overhead) {
			cout << "sdsl_v1 space" << endl;
			print_overhead_line("input:", N_bits);
			print_separator();
			print_overhead_line("bv", bv_bits);
			print_overhead_line("rank_v1", r_bits);
			print_separator();
